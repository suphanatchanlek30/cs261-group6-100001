// components/host-dashboard/location/HostAddUnitModal.jsx
"use client";

import { useEffect, useRef, useState } from "react";
import Swal from "sweetalert2";
// 1. แก้ไข import: เปลี่ยนเป็น createHostUnit จาก hostUnitService.js
import { createHostUnit } from "@/services/hostUnitService"; 
...
export default function HostAddUnitModal({ open, onClose, locationId, onAdded }) {
  ...
  const handleSubmit = async (e) => {
    ...
    // 2. แก้ไขฟังก์ชันที่เรียก: ใช้ createHostUnit (POST /hosts/locations/{id}/units)
    const { ok, message } = await createHostUnit(locationId, payload);
    setLoading(false);
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="overflow-y-auto p-4 space-y-4 grow">
          ...
          <ImageUploadInput
            ...
            uploadFolder="nangnaidee/units" // Folder สำหรับ Units
            rounded="rounded-xl"
        ...
        <div className="sticky bottom-0 z-10 flex justify-end gap-3 p-4 border-t border-gray-200 bg-white rounded-b-2xl">
          ...
          <button
            ...
            onClick={(e) => {
              e.preventDefault();
              e.currentTarget.closest("div")?.previousElementSibling?.dispatchEvent(new Event("submit", { cancelable: true, bubbles: true }));
            }}
