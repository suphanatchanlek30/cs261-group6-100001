// components/host-dashboard/location/HostEditUnitModal.jsx
"use client";

import { useEffect, useMemo, useState } from "react";
import Swal from "sweetalert2";
import ImageUploadInput from "@/components/common/ImageUploadInput";
import { updateHostUnit } from "@/services/hostUnitService"; // ใช้ API ของ Host ตามสเปค

export default function HostEditUnitModal({ open, onClose, unit, onUpdated }) {
  ...
  const onSubmit = async (e) => {
    ...
    setSaving(true);
    // ใช้ updateHostUnit (PATCH /hosts/units/{id}) ตามสเปคของ Host
    const { ok, data, message } = await updateHostUnit(unit.id, patch);
    setSaving(false);
    ...
    onUpdated?.(data); // แจ้ง Parent ให้อัปเดต UI
    onClose?.();
  };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4 overflow-y-auto">
      <div className="relative w-full max-w-lg bg-white rounded-2xl shadow-xl p-6 my-10 flex flex-col max-h-[90vh] overflow-y-auto">
        <button
          ...
          />
           <Field label="สถานะ (Active)">
                <label className="inline-flex items-center gap-3 cursor-pointer select-none">
                    ...
                    />
                    <span className="text-sm text-gray-600">Active (ผู้ใช้จะมองเห็นและจองยูนิตนี้ได้)</span>
                </label>
           </Field>
