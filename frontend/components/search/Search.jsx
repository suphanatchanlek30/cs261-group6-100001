...
const ChevronDownIcon = () => (
    <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 ml-auto text-neutral-500 transition-transform group-open:rotate-180" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3">
                            ...
                            <details ref={detailsRef} className="group">
                                ...
                                <div className="absolute right-0 mt-2 w-48 origin-top-right rounded-xl border bg-white p-1 shadow-lg ring-1 ring-black/5 focus:outline-none z-50">
                                    {distanceOptions.map((d) => (
                                        <button
                                            key={d} 
                                            onClick={() => { setDistance(d); detailsRef.current?.removeAttribute("open"); }}
                                            className={`block w-full rounded-lg px-4 py-2 text-left text-sm ${ distance === d ? "bg-violet-500 text-white" : "text-neutral-700 hover:bg-violet-100" }`}
                                        >
                                            {d} km
                                        </button>
                                    ))}
